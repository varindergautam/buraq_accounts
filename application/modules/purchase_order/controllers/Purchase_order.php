...
class Purchase_order extends MX_Controller
{
    ...
    public function to_purchase_order($quot_id = null)
    {
        ...
        $vatortax              = $this->quotation_model->vat_tax_setting();
        if ($vatortax->fixed_tax == 1) {
            $data['page']            = "quotation_to_purchase_order";
        }
        if ($vatortax->dynamic_tax == 1) {
            $data['page']          = "quotation_to_purchase_order_dynamic";
        }
        echo modules::run('template/layout', $data);
    }
    ...
    public function save_purchase_order()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $tablecolumn = $this->db->list_fields('purchase_order_taxinfo');
            $num_column  = count($tablecolumn) - 4;
            $fixordyn    = $this->db->select('*')->from('vat_tax_setting')->get()->row();
            $is_fixed    = '';
            $is_dynamic  = '';
    ...
    public function purchase_order_to_sale_order($quot_id = null)
    {
        $vat_tax_info   = $this->quotation_model->vat_tax_setting();
        $data['quot_main']       = $this->purchase_order_model->purchase_order_main_edit($quot_id);
        ...
        $vatortax              = $this->quotation_model->vat_tax_setting();
        if ($vatortax->fixed_tax == 1) {
            $data['page']            = "purchase_order_to_sale_order";
        }
        if ($vatortax->dynamic_tax == 1) {
            $data['page']          = "purchase_order_to_sale_order_dynamic";
        }
        echo modules::run('template/layout', $data);
    }

    public function add_purchase_order_to_sale_order()
    {
        $this->form_validation->set_rules('customer_id', display('customer_name'), 'required|max_length[50]');
        $this->form_validation->set_rules('qdate', display('quotation_date'), 'required|max_length[50]');
        $this->form_validation->set_rules('expiry_date', display('expiry_date'), 'required|max_length[50]');
    ...
    public function purchase_order_to_delivery($quot_id = null)
    {
        $data['type'] = isset($_GET['type']) ? $_GET['type'] : NULL;
        $vat_tax_info   = $this->quotation_model->vat_tax_setting();
        $data['quot_main']       = $this->purchase_order_model->purchase_order_main_edit($quot_id);
        ...
        if ($vatortax->fixed_tax == 1) {
            ...
            $data['page']            = "purchase_order_to_delivery";
        }
        if ($vatortax->dynamic_tax == 1) {
            $data['page']          = "purchase_order_to_delivery_dynamic";
        }
        echo modules::run('template/layout', $data);
    }
    ...
    public function to_sales($quot_id = null)
    {
        ...
        if ($vatortax->fixed_tax == 1) {
            ...
            $data['page']            = "purchase_order_to_sale";
        }
        if ($vatortax->dynamic_tax == 1) {
            $data['page']          = "purchase_order_to_sale_dynamic";
        }
        echo modules::run('template/layout', $data);
    }

    public function save_purchase_order_to_sales()
    {
        ...
        if ($finyear <= 0) {
            ...
            if ($this->form_validation->run() === true) {
                ...
                redirect("manage_purchase_order/" . $quotation_id);
            }
        }
    }
