...
class Product extends MX_Controller {
   ...
   // category part
    ...
    public function check_min_sale_price($min_sale_price)
    {
        $supplier_price = $this->input->post('supplier_price', TRUE);

        if ($min_sale_price < $supplier_price) {
            $this->form_validation->set_message('check_min_sale_price', 'The Minimum Sale Price must be greater than or equal to the Supplier Price.');
            return false;
        } else {
            return true;
        }
    ...
    public function bdtask_product_form($id = null){
       ...
       $data['title'] = display('add_product');
        #-------------------------------#
//        $this->form_validation->set_rules('min_sale_price',display('min_sale_price'),'required|max_length[200]');
...
//        $this->form_validation->set_rules('min_sale_price', 'Minimum Sale Price','callback_check_min_sale_price');
        $min_sale_price = $this->input->post('min_sale_price',TRUE);
        $supplier_price = $this->input->post('supplier_price',TRUE);
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (empty($id)) {
                 if($min_sale_price > $supplier_price){
                     ...
                     $this->session->set_flashdata('exception', display('Minimum_sal_price_should_be_Grater_than_Cost_price'));
                    }
                }
                else {
                    $this->session->set_flashdata('exception', set_message('check_min_sale_price', 'The Minimum Sale Price must be greater than or equal to the Supplier Price.'));
                }
                ...
                if ($min_sale_price > $supplier_price) {
                    ...
                    if ($this->product_model->update_product($postData)) {
                        ...
                        $this->session->set_flashdata('exception', 'The Minimum Sale Price must be greater than or equal to the Supplier Price.');
//                    $this->form_validation->set_message('check_min_sale_price', 'The Minimum Sale Price must be greater than or equal to the Supplier Price.');
