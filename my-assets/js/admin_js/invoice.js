...
function addInputField_invoice(t) {
    ...
    for(var i=0;i<taxnumber;i++){
        ...
        // var hiddenInputField = "<input type='hidden' name='min_sale_price[]' id='supplier_price_" + count + "' value='' />";
    }
    if (count == limits)
        ...
        "' name='product_id[]' id='SchoolHiddenId'/></td><td><input type='text' name='desc[]'' class='form-control text-right ' tabindex='" +
         ...
         count + " form-control text-right'  placeholder='0.00' min='0' tabindex='" + tab3 + "'/></td><td><input type='text' name='product_rate[]' onkeyup='bdtask_invoice_quantity_calculate(" +
            count + "); validateMinValue(this);' onchange='bdtask_invoice_quantity_calculate(" + count + ");validateMinValue(this);' id='price_item_" + count + "' class='common_rate price_item" +
            count + " form-control text-right' required placeholder='0.00' min='0' tabindex='" + tab4 + "'/> <input type='hidden' name='min_sale_price[]' id='min_sale_price_" + count + "' value='' /><span id='minValueMsg_" + count + "' style='display: none; color: red;'></span></td><td><input type='text' name='discount[]' onkeyup='bdtask_invoice_quantity_calculate("
...
function addInputField_invoice_dynamic(t) {
    ...
    if (count == limits)
        ...
        "' name='product_id[]' id='SchoolHiddenId'/></td><td><input type='text' name='desc[]'' class='form-control text-right ' tabindex='" +
         ...
         count + " form-control text-right' required placeholder='0.00' min='0' tabindex='" + tab4 + "'/> +  hiddenInputField +</td><td><input type='text' name='discount[]' onkeyup='bdtask_invoice_quantity_calculate("
         + count + ");' onchange='bdtask_invoice_quantity_calculate(" + count + ");' id='discount_" + count + "' class='form-control text-right common_discount' placeholder='0.00' min='0' tabindex='" + tab5 +
         "'><input type='hidden' value='' name='discount_type' id='discount_type_" + count + "'></td><td><input type='text' name='discountvalue[]'  id='discount_value_" + count +
   ...
   var frm = $("#update_invoice");
    ...
    "use strict";
     function invoice_productList(sl) {
         ...
         var min_sale_price_ = 'min_sale_price_'+sl;
    ...
    var options = {
        open: function(event, ui) {
            var maxOptionsToShow = 5; // Maximum number of options to display without scrolling
            var $menu = $(this).data("ui-autocomplete").menu.element;

            if ($menu.children('li').length > maxOptionsToShow) {
                var maxHeight = 0;
                $menu.children('li:lt(' + maxOptionsToShow + ')').each(function() {
                ...
                $.ajax
                   ({
                        ...
                        success: function(data)
                        {
                            ...
                            // var minSalePrice = parseFloat(obj.min_sale_price);
                            // console.log(minSalePrice);
                           $('.'+txclass).val(obj.taxdta[i]);
                            }
                            $('.'+priceClass).val(obj.price);
                            // $('.'+min_sale_price_).val(obj.min_sale_price);
                            $('.'+unit).val(obj.unit);
                            $('.'+tax).val(obj.tax);
                            ...
                            $('#'+min_sale_price_).val(obj.min_sale_price);
                            console.log(i);
                            console.log("Value to set for min_sale_price: " + obj.min_sale_price);
