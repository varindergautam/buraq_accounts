...
function addInputField(t) {
    ...
    if (count == limits)
        ...
        "'/></td><input type='hidden' name='available_quantity[]' id='' class='form-control text-right common_avail_qnt available_quantity_" + count +
        "' value='0' readonly='readonly' /><input class='form-control text-right common_name unit_" + count +
...
function quantity_calculate(item) {
    var denis_quantity = $("#total_qntt_" + item).val();
...
if (quantity > 0 || discount > 0 || vat_percent > 0) {
        ...
        if (dis_type == 1) {
            ...
            var price = denis_quantity * price_item;
        ...
        var n = denis_quantity * price_item;
        var c = denis_quantity * price_item * total_tax;
        $("#total_price_" + item).val(n),
        $("#all_tax_" + item).val(c)
    }

    $("#total_qntt_" + item).val(denis_quantity);
...
function calculateSum() {
   ...
   for(var i=0;i<taxnumber;i++){
    ...
    var selectedValue = $('#cardTypeSelect').val();
    if(selectedValue === '0'){
        $('#paidAmount').val('0');
        $('#pamount_by_method').val('0');
        $('#dueAmmount').val(grnt_totals);
    }
}
